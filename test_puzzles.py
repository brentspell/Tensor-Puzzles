...
def diff(a: TT["i"], i: int) -> TT["i"]:
    return where(arange(a.shape[0]) == 0, a, a - a[arange(a.shape[0]) - 1])
...
def vstack(a: TT["i"], b: TT["i"]) -> TT[2, "i"]:
    return (1 - arange(2))[:, None] @ a[None] + arange(2)[:, None] @ b[None]
...
def roll(a: TT["i"], i: int) -> TT["i"]:
    return a[(arange(a.shape[0]) + 1) % a.shape[0]]
...
def flip(a: TT["i"], i: int) -> TT["i"]:
    return a[a.shape[0] - arange(a.shape[0]) - 1]
...
def compress(g: TT["i", bool], v: TT["i"], i:int) -> TT["i"]:
    return eye(v.shape[0])[:, :sum(g * 1)] @ v[g]
...
def pad_to(a: TT["i"], i: int, j: int) -> TT["j"]:
    return eye(where(i > j, i, j))[:j, :i] @ a
...
def bincount(a: TT["i"], j: int) -> TT["j"]:
    return sum((a[None] == arange(j)[:, None]) * 1)
